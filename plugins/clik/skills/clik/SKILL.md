...
The `clik` plugin, enabled at user scope, already provides — in every project — all the workflow skills (`/start-issue`, `/done`, `/pr-review`, `/tdd`, `/ship`, …), the specialist agents (`@code-reviewer`, `@security-reviewer`, `@architect`, …), and the safety/format/session hooks. **Do not copy skills, agents, or hooks into the project.** Your job here is only to tailor the *per-project* config: a tight `CLAUDE.md`, the right `rules/`, and accurate `permissions`.
...
Copy the universal rules from `${CLAUDE_PLUGIN_ROOT}/template/rules/` that apply, then tailor each rule's `paths:` frontmatter to the project's actual directories. If the domain has non-obvious practices worth enforcing, write them as a new `.claude/rules/<domain>.md` (e.g. `cuda.md`, `data-viz.md`) with `paths:` scoped to the relevant files so they only cost tokens near that code. Drop rules that don't apply (no frontend → no `frontend.md`; no DB → no `database.md`).
...
## Phase 4: Review, budget check, summary
